...
func chooseIPFromHostInterfaces(nw networkInterfacer) (net.Interface, net.IP, error) {
	intfs, err := nw.Interfaces()
	if err != nil {
		return net.Interface{}, nil, err
	}
	if len(intfs) == 0 {
		return net.Interface{}, nil, fmt.Errorf("no interfaces found on host.")
	}
	for _, family := range []AddressFamily{familyIPv4, familyIPv6} {
		...
		for _, intf := range intfs {
			...
			for _, addr := range addrs {
				...
				if err != nil {
					return intf, nil, fmt.Errorf("Unable to parse CIDR for interface %q: %s", intf.Name, err)
				}
				if !memberOf(ip, family) {
	...
	return net.Interface{}, nil, fmt.Errorf("no acceptable interface with global unicast address found on host")
}
...
func ChooseHostInterface() (net.Interface, net.IP, error) {
...
func chooseHostInterfaceFromRoute(routes []Route, nw networkInterfacer) (net.Interface, net.IP, error) {
	for _, family := range []AddressFamily{familyIPv4, familyIPv6} {
		...
		for _, route := range routes {
			...
			if finalIP != nil {
				...
				inf, err := net.InterfaceByName(route.Interface)
				if err != nil {
					return net.Interface{}, nil, err
				}
				return *inf, finalIP, nil
			}
		}
	}
	klog.V(4).Infof("No active IP found by looking at default routes")
	return net.Interface{}, nil, fmt.Errorf("unable to select an IP from default routes.")
}

// If bind-address is usable, return it directly
// If bind-address is not usable (unset, 0.0.0.0, or loopback), we will use the host's default
// interface.
func ChooseBindAddress(bindAddress net.IP) (net.Interface, net.IP, error) {
	var inf net.Interface
	if bindAddress == nil || bindAddress.IsUnspecified() || bindAddress.IsLoopback() {
		tmpInf, hostIP, err := ChooseHostInterface()
		if err != nil {
			return inf, nil, err
		}
		bindAddress = hostIP
		...
		ifaces, err := NetworkInterface{}.Interfaces()
		if err != nil {
			return inf, nil, err
		}
		for _, iface := range ifaces {
			...
			for _, addr := range addrs {
				if strings.HasPrefix(addr.String(), bindAddress.String()) {
					return iface, bindAddress, nil
				}
			}
		}
	}
	if inf.Name == "" {
		return inf, bindAddress, errors.Errorf("Not found interface by address %s", bindAddress)
	}
	return inf, bindAddress, nil
}
