...
var MeterServiceComponent IComponent = NewMeterService()
...
func NewMeterService() *MeterService {
	m := new(MeterService)
	m.BaseComponent = NewBaseComponent(onecloud.ComponentType("meter-service"), m)
	return m
}

func (m MeterService) NewService(oc *onecloud.OnecloudCluster) *corev1.Service {
	//return NewSinglePortService(m.GetComponentType(), oc, MeterServicePort)
	return NewNodePortService(m.GetComponentType(), oc, MeterServicePort)
}

func (m MeterService) NewDeployment(oc *onecloud.OnecloudCluster) (*apps.Deployment, error) {
...
func (m MeterService) NewConfigMap(oc *onecloud.OnecloudCluster, cCfg *OnecloudComponentsConfig) (*corev1.ConfigMap, error) {
	cfg := cCfg.MeterConfig
	config := NewJavaDBConfig(oc, cfg)
	return NewConfigMapByTemplate(m.GetComponentType(), oc, MeterServiceConfigTemplate, config)
}

func (m MeterService) NewCloudUser(cfg *OnecloudComponentsConfig) *onecloud.CloudUser {
	return &cfg.MeterConfig.CloudUser
}

func (m MeterService) NewDBConfig(cfg *OnecloudComponentsConfig) *onecloud.DBConfig {
