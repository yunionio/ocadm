...
var MeterComponent IComponent = NewMeter()
...
func NewMeter() *Meter {
	m := new(Meter)
	m.BaseComponent = NewBaseComponent(onecloud.ComponentType("meter"), m)
	return m
}

func (c Meter) NewService(oc *onecloud.OnecloudCluster) *corev1.Service {
	return NewNodePortService(c.GetComponentType(), oc, MeterPort)
}

func (c Meter) NewDeployment(oc *onecloud.OnecloudCluster) (*apps.Deployment, error) {
		...
		return []corev1.Container{
			{
				...
				WorkingDir:   JAVA_APP_WORKING_DIR,
...
func (c Meter) NewConfigMap(oc *onecloud.OnecloudCluster, cCfg *OnecloudComponentsConfig) (*corev1.ConfigMap, error) {
	cfg := cCfg.MeterConfig
	config := NewJavaDBConfig(oc, cfg)
	return NewConfigMapByTemplate(c.GetComponentType(), oc, MeterConfigTemplate, config)
}

func (c Meter) NewCloudUser(cfg *OnecloudComponentsConfig) *onecloud.CloudUser {
	return &cfg.MeterConfig.CloudUser
}

func (c Meter) NewDBConfig(cfg *OnecloudComponentsConfig) *onecloud.DBConfig {
	return &cfg.MeterConfig.DB
}

func (c Meter) NewCloudEndpoint() *CloudEndpoint {
	return NewHTTPCloudEndpoint(ServiceNameMeter, ServiceTypeMeter, MeterPort, "api/v1")
}
